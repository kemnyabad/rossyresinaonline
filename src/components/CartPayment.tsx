...
const CartPayment = ({ email, onConfirm, confirmLabel = "Confirmar pedido por WhatsApp" }: Props) => {
  ...
  const handleConfirm = () => {
    ...
    if (!contactPhone) {
      alert("No se ha configurado el numero de contacto.");
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="text-sm text-gray-700">
        <p className="mt-2">Paga al numero Yape <span className="font-semibold">{yapeNumber}</span> y confirma por WhatsApp.</p>
        {holder && <p className="mt-1">Titular: <span className="font-semibold">{holder}</span></p>}
        <p className="mt-1">En el mensaje se incluira el detalle de tu pedido.</p>
      </div>
