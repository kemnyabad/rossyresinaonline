...
export default function CapacitacionesPage({ cursos }: Props) {
  ...
  return (
    <>
      ...
      <main className="font-bodyFont min-h-screen bg-[#f5f5f7] text-[#1f2933]">
        <header className="border-b border-slate-200 bg-white">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
            ...
            <a
              href={whatsappUrl}
              className="inline-flex items-center gap-2 rounded-lg bg-[#25d366] px-4 py-2 text-sm font-bold text-white shadow-sm transition hover:brightness-95"
            >
        ...
        <section className="border-b border-slate-200 bg-gradient-to-br from-[#c21885] to-[#8a1063] text-white">
          <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-xs font-bold uppercase tracking-wide text-white ring-1 ring-white/25">
              <AcademicCapIcon className="h-4 w-4" />
              Escuela Rossy Resina
            </span>
        ...
        <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
          {cursos.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500 shadow-sm">
              ...
              <a
                href={whatsappUrl}
                className="mt-4 inline-flex rounded-lg bg-[#25d366] px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:brightness-95"
              >
                ...
                {cursos.map((curso) => (
                  ...
                  >
                    <div className="grid lg:grid-cols-[220px_1fr_280px]">
                      <div className="relative flex h-44 items-center justify-center bg-slate-100 lg:h-full">
                        {curso.imagen ? (
                      ...
                      <div className="p-5">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="rounded-full bg-[#eef2ff] px-3 py-1 text-xs font-bold uppercase tracking-wide text-[#3730a3]">
                            {curso.modalidad}
                          </span>
                        ...
                        <h3 className="mt-3 text-xl font-black leading-tight text-slate-950">{curso.nombre}</h3>
                        {curso.descripcion ? (
                          <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600">{curso.descripcion}</p>
                        ) : null}

                        <div className="mt-4 flex flex-col gap-2 border-t border-slate-100 pt-4 text-sm text-slate-700 sm:flex-row sm:flex-wrap sm:gap-x-6">
                          <p className="flex items-center gap-2">
                            <ClockIcon className="h-4 w-4 shrink-0 text-[#d4001a]" />
                            Duración: <span className="font-semibold text-slate-900">{curso.duracionHoras} h</span>
                          </p>
                      ...
                      <div className="flex flex-col gap-4 border-t border-slate-100 bg-[#fafafb] p-5 lg:border-l lg:border-t-0">
                        <div>
                          <p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Inversión</p>
                          <div className="mt-1 flex items-baseline gap-2">
                            <span className="text-3xl font-black text-red-600">S/ {curso.precio.toFixed(2)}</span>
                            {curso.precioAnterior ? (
                              <span className="text-sm text-slate-400 line-through">
                                S/ {curso.precioAnterior.toFixed(2)}
                              </span>
                            ) : null}
                          </div>
                        </div>
      ...
      {activeFecha && (
        ...
        >
          <div className="font-bodyFont w-full max-w-md rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <p className="flex items-center gap-2 font-black text-slate-900">
                <CalendarDaysIcon className="h-5 w-5 text-[#c21885]" />
                Reservar cupo
              </p>
              ...
              {sent ? (
                ...
                <form onSubmit={submit} className="grid gap-4">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                    <p className="font-bold text-slate-900">{activeFecha.curso.nombre}</p>
                  ...
                  <div className="grid gap-3">
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wide text-slate-500">Nombre completo *</label>
                      <input
                        value={form.nombre}
                        onChange={(e) => setForm({ ...form, nombre: e.target.value })}
                        className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-[#c21885] focus:ring-1 focus:ring-[#c21885]"
                        placeholder="Tu nombre"
                      />
                    </div>
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wide text-slate-500">Correo electrónico *</label>
                      <input
                        type="email"
                        value={form.email}
                    ...
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wide text-slate-500">WhatsApp / Teléfono *</label>
                      <input
                        value={form.telefono}
                    ...
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wide text-slate-500">Mensaje adicional (opcional)</label>
                      <textarea
                        value={form.notas}
                        onChange={(e) => setForm({ ...form, notas: e.target.value })}
                        className="mt-1.5 min-h-[70px] w-full resize-none rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-[#c21885] focus:ring-1 focus:ring-[#c21885]"
                  ...
                  <button
                    type="submit"
                    disabled={sending}
                    className="h-11 rounded-lg bg-[#c21885] text-sm font-bold text-white shadow-sm transition hover:brightness-95 disabled:opacity-60"
                  >
